1- run all unit tests and make sure they all pass
2- Do a thorough cleanup:
- remove debug logs and debug or test files
- remove unused code
- fix linting errors
- oraganize or refactor if it helps to have a less cluttered and more maintainable code

You primary goal should be a clean and minimal code base.
At the end, report your changes briefly, the total number of new lines in the git working directory, and the number of lines you added removed.