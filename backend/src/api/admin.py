...
async def test_openai_api(
    ...
    if not AIConfig.is_enabled():
        return OpenAIDiagnosticResponse(
            ...
            request_details=DiagnosticRequestDetails(
                ...
                system_length=len(request.system_message or "You are a helpful assistant.")
            ),
    ...
    try:
        ...
        # Calculate metrics
        tokens_per_second = (completion_tokens / (response_time_ms / 1000.0)) if response_time_ms > 0 else 0.0

        # Estimate cost using centralized pricing configuration
        estimated_cost = OpenAIPricing.estimate_cost(model, prompt_tokens, completion_tokens)
