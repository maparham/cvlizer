...
async def get_ai_usage_by_operation(
    ...
    try:
        ...
        operations = get_usage_by_operation(
            db=db,
            start_date=start_dt,
            end_date=end_dt
        )
...
async def get_ai_usage_timeline(
    start_date: Optional[str] = Query(None, description="Start date for filtering (YYYY-MM-DD format)"),
    end_date: Optional[str] = Query(None, description="End date for filtering (YYYY-MM-DD format)"),
    granularity: str = Query("day", pattern="^(day|week|month|hour)$", description="Timeline granularity"),
    db: Session = Depends(get_db),
    admin_user: User = Depends(require_admin_allow_impersonating)
):
    """
...
async def delete_all_ai_usage_logs(
    ...
    try:
        ...
        logger.info(f"Admin {admin_user.email} deleted all AI usage logs ({deleted_count} records)")
...
async def export_all_ai_usage_logs(
    ...
    try:
        ...
        if start_dt and end_dt:
            query = query.filter(
                and_(
                    AIUsageLog.timestamp >= start_dt,
                    AIUsageLog.timestamp <= end_dt
                )
            )
        elif start_dt:
        ...
        for usage_log, user_email in results:
            writer.writerow([
                sanitize_csv_field(usage_log.timestamp.isoformat() if usage_log.timestamp else ''),
