...
class CreateHistoryRequest(BaseModel):
    ...
    is_initial: bool = Field(
        False, description="Whether this is the initial version (undeletable)"
    )
...
class TextDiffData(BaseModel):
    inline_diff: Optional[str] = Field(
        None, description="Inline diff with character-level highlighting"
    )
    word_diff: List[str] = Field(
        default_factory=list, description="Word-level diff summary"
    )
...
class DiffChangeResponse(BaseModel):
    type: str = Field(
        ..., description="Type of change (field_changed, item_added, item_removed, etc.)"
    )
    ...
    details: List[str] = Field(
        default_factory=list, description="Detailed change information"
    )
    text_diff: Optional[TextDiffData] = Field(
        None, description="Detailed text diff for text fields"
    )
...
class CVDiffResponse(BaseModel):
    changes: List[DiffChangeResponse] = Field(
...
async def create_history_entry(
    ...
    cv = db.query(CV).filter(and_(CV.id == cv_id, CV.user_id == current_user.id)).first()
...
async def get_cv_history(
    ...
    history_entries = (
        db.query(CVHistory)
        .filter(CVHistory.cv_id == cv_id)
        .order_by(desc(CVHistory.created_at))
        .offset(offset)
        .limit(limit)
        .all()
    )
    ...
    return [
        HistoryEntryResponse(**entry.to_frontend_format()) for entry in history_entries
    ]
...
async def get_history_entry(
    ...
    history_entry = (
        db.query(CVHistory)
        .filter(and_(CVHistory.id == entry_id, CVHistory.cv_id == cv_id))
        .first()
    )
...
async def clear_cv_history(
    ...
    deleted_count = (
        db.query(CVHistory)
        .filter(and_(CVHistory.cv_id == cv_id, CVHistory.is_initial == False))
        .delete()
    )
    db.commit()

    return {
        "message": f"Cleared {deleted_count} history entries (preserved initial version)"
    }
...
async def restore_cv_version(
    ...
    return {"message": "CV restored successfully", "cv": cv.to_response_dict()}
...
async def get_version_diff(
    ...
    compare_to: Optional[
        str
    ] = None,  # If not provided, compare to original version or previous version
    ...
    if compare_to:
        ...
        if force_previous:
            ...
            all_entries = (
                ...
                .order_by(CVHistory.created_at.asc())
                .all()
            )
            ...
            if not compare_entry:
                ...
                all_entries = (
                    ...
                    .filter(
                        and_(
                            CVHistory.cv_id == cv_id, CVHistory.user_id == current_user.id
                        )
                    )
                    .order_by(CVHistory.created_at.asc())
                    .all()
    ...
    diff_result = cv_diff_service.compute_diff(
        compare_entry.cv_data, target_entry.cv_data  # old data  # new data
    )
