...
class CVHistory(Base):
    ...
    change_type = Column(String(50), nullable=False)  # Type of change that triggered this snapshot
    description = Column(Text, nullable=True)  # Human-readable description
    label = Column(String(255), nullable=True)  # Optional user-provided label
    
    # Metadata
    is_automatic = Column(Boolean, default=True, nullable=False)  # Whether this was an auto-snapshot
    is_initial = Column(Boolean, default=False, nullable=False)  # Whether this is the initial version (undeletable)
