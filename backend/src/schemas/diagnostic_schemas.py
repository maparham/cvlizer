...
class OpenAIDiagnosticRequest(BaseModel):
    ...
    system_message: Optional[str] = Field(
        default="You are a helpful assistant.", description="System message for the model"
    )
    max_tokens: Optional[int] = Field(
        default=500, ge=1, le=4000, description="Maximum tokens in response"
    )
    temperature: Optional[float] = Field(
        default=0.7, ge=0.0, le=2.0, description="Temperature for response randomness"
    )
...
class OpenAIDiagnosticResponse(BaseModel):
    ...
    response_text: Optional[str] = Field(
        default=None, description="Response from the model"
    )
    api_type: str = Field(
        default="responses", description="API type used (always 'responses')"
    )
    metrics: Optional[DiagnosticMetrics] = Field(
        default=None, description="Performance metrics"
    )
    request_details: DiagnosticRequestDetails = Field(
