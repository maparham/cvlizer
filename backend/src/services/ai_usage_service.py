...
def get_usage_stats(
    ...
    try:
        ...
        # Apply filters
        query = query.filter(
            and_(AIUsageLog.timestamp >= start_date, AIUsageLog.timestamp <= end_date)
        )
        ...
        return {
            ...
            "average_prompt_tokens_per_operation": float(
                stats.avg_prompt_tokens_per_operation or 0.0
            ),
    ...
    try:
        ...
        results = (
            db.query(
                ...
                func.count(AIUsageLog.id).label("operation_count"),
                func.avg(AIUsageLog.total_tokens).label("avg_tokens_per_operation"),
                func.avg(AIUsageLog.prompt_tokens).label(
                    "avg_prompt_tokens_per_operation"
                ),
                func.avg(AIUsageLog.completion_tokens).label(
                    "avg_completion_tokens_per_operation"
                ),
        ...
        return [
            {
                ...
                "average_tokens_per_operation": float(
                    result.avg_tokens_per_operation or 0.0
                ),
                "average_prompt_tokens_per_operation": float(
                    result.avg_prompt_tokens_per_operation or 0.0
                ),
                "average_completion_tokens_per_operation": float(
                    result.avg_completion_tokens_per_operation or 0.0
                ),
            }
            for result in results
        ]
...
def get_usage_logs(
    ...
    try:
        ...
        # Build base query
        query = (
            db.query(AIUsageLog, User.email)
            .join(User, AIUsageLog.user_id == User.id)
            .filter(
                and_(AIUsageLog.timestamp >= start_date, AIUsageLog.timestamp <= end_date)
            )
        ...
        results = (
            query.order_by(desc(AIUsageLog.timestamp)).offset(offset).limit(limit).all()
        )
