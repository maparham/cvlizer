...
# Credit line appended to every exported CV. Opt-out per CV via
# cvs.show_ai_attribution.
AI_ATTRIBUTION_TEXT = "Generated by rahkar.pro"

# Credit line attached to the job-tailored section only (the one written
# against the job's requirements). Shares the same per-CV opt-out. The model
# that actually produced the section is appended when it is recorded.
SECTION_ATTRIBUTION_TEXT = "Generated by rahkar.pro AI agent"

# Vendor prefixes on model ids ("openai/gpt-5") carry no reader value.
_MODEL_VENDOR_PREFIXES = ("openai/", "anthropic/", "google/", "meta-llama/", "mistralai/")

# Leading token -> display casing. Anything unmatched is shown verbatim, which
# is honest: a raw id beats a wrong guess at a marketing name.
...
def _format_model_label(model_used: Optional[str]) -> str:
    """Turn a stored model id into something a CV reader can parse.

    "openai/gpt-5" -> "GPT-5"; "claude-opus-5" -> "Claude-opus-5". Unknown ids
    pass through unchanged rather than being guessed at.
    """
    ...
    lowered = raw.lower()
    for prefix in _MODEL_VENDOR_PREFIXES:
        if lowered.startswith(prefix):
            raw = raw[len(prefix) :]
            break

    # Drop routing suffixes such as ":free" or ":nitro".
    raw = raw.split(":", 1)[0].strip()
    if not raw:
        return ""

    for token, display in _MODEL_DISPLAY_PREFIXES.items():
        if raw.lower().startswith(token):
            return display + raw[len(token) :]
    return raw


def _section_attribution_text(model_used: Optional[str]) -> str:
    """Credit line for the job-tailored section, naming the model when known."""
    label = _format_model_label(model_used)
    return f"{SECTION_ATTRIBUTION_TEXT} ({label})" if label else SECTION_ATTRIBUTION_TEXT
...
def _section(
    title: str, body: str, template_name: str, title_footnote: Optional[str] = None
) -> str:
    """Create a section using the template-specific macro, with fallback.

    ``title_footnote`` attaches a numbered footnote to the section title. The
    marker and \\footnotetext are emitted together at the top of the section, so
    LaTeX always sets the note at the foot of the page where the section begins,
    even when the section body spills onto the next page.

    \\footnotemark plus \\footnotetext is used rather than a plain \\footnote so
    the note body stays outside the heading's bold/coloured group.
    """
    escaped_title = _tex_escape(title)
    # hidelinks stops hyperref colouring the marker link-blue; it then inherits
    # the heading's own colour instead of looking like a clickable link.
    marker = (
        "\\begingroup\\hypersetup{hidelinks}\\footnotemark\\endgroup"
        if title_footnote
        else ""
    )
    note = f"\\footnotetext{{{_tex_escape(title_footnote)}}}\n" if title_footnote else ""
    section_cmd = SECTION_CMD_BY_TEMPLATE.get(template_name)
    if section_cmd:
        return (
            f"\\needspace{{5\\baselineskip}}\n"
            f"\\{section_cmd}{{{escaped_title}{marker}}}\n"
            f"{note}{body}\\vspace{{0.5\\baselineskip}}\n"
        )
    # Fallback legacy styling
    return (
        f"\\needspace{{4\\baselineskip}}\\noindent{{\\Large\\textbf{{{escaped_title}{marker}}}}}\\\\[-1ex]\n"
        f"\\rule{{\\textwidth}}{{0.4pt}}\\\\[0.5ex]\n{note}{body}\\vspace{{0.5\\baselineskip}}\n"
    )
...
def _format_ai_attribution() -> str:
    """Format the closing rahkar.pro credit line.
...
def _generate_from_template(
    ...
    Args:
        ...
        show_ai_attribution: Append the rahkar.pro credit line at the end of
            the document
    ...
    why_fit_meta = (parsed.get("why_good_fit_metadata") or {}) if parsed else {}
    section_credit = _section_attribution_text(
        why_fit_meta.get("model_used") if isinstance(why_fit_meta, dict) else None
    )
            ...
            if section_type == "custom":
                ...
                if custom_item:
                    ...
                    if raw:
                        ...
                        is_ai_section = section_id == "why_good_fit"
                        content_parts.append(
                            _section(
                                ...
                                title_footnote=(
                                    section_credit
                                    if show_ai_attribution and is_ai_section
                                    else None
                                ),
                            )
                        )
        ...
        if why_fit_content := _format_why_good_fit(why_fit):
            content_parts.append(
                _section(
                    ...
                    why_fit_content,
                    template_name,
                    title_footnote=(section_credit if show_ai_attribution else None),
                )
            )
