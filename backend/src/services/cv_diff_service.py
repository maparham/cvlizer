...
class CVDiffService:
    ...
    def compute_diff(
        ...
        return {"changes": changes, "summary": summary, "total_changes": len(changes)}
    ...
    def _compute_simple_diff(
        ...
        for section in sections_to_compare:
            ...
            if section in [
                ...
                # Array sections
                changes.extend(
                    self._compare_array_section(
                        section, old_section or [], new_section or []
                    )
                )
            elif section == "skills":
                # Skills object
    ...
    def _compare_array_section(
        self, section: str, old_items: List[Dict], new_items: List[Dict]
    ) -> List[Dict[str, Any]]:
                    ...
                    changes.append(
                        {
                            ...
                            "text_diff": (
                                text_diffs[0] if len(text_diffs) == 1 else None
                            ),  # Only include text diff if single text change
                            "icon": "edit",
                            "color": "warning",
                        }
                    )
    ...
    def _compare_object_section(
        self, section: str, old_obj: Optional[Dict], new_obj: Optional[Dict]
    ) -> List[Dict[str, Any]]:
    ...
    def _compare_item_fields(
        self, old_item: Dict[str, Any], new_item: Dict[str, Any], section: str
    ) -> List[Dict[str, Any]]:
    ...
    def _is_text_field(self, field: str, old_value: Any, new_value: Any) -> bool:
        ...
        if (
            ...
            or "description" in field.lower()
            or "content" in field.lower()
        ):
    ...
    def _generate_text_diff(self, old_text: str, new_text: str) -> Dict[str, Any]:
        ...
        char_additions = (
            len(new_text) - len(old_text) if len(new_text) > len(old_text) else 0
        )
        char_deletions = (
            len(old_text) - len(new_text) if len(old_text) > len(new_text) else 0
        )
    ...
    def _generate_inline_diff(self, old_text: str, new_text: str) -> str:
        ...
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            if tag == "equal":
                ...
                result.append(
                    f'<span style="background-color: #ffcdd2; text-decoration: line-through;">{old_text[i1:i2]}</span>'
                )
            elif tag == "insert":
                # Inserted text (green background)
                result.append(
                    f'<span style="background-color: #c8e6c9; font-weight: bold;">{new_text[j1:j2]}</span>'
                )
            elif tag == "replace":
    ...
    def _generate_word_diff(self, old_text: str, new_text: str) -> List[str]:
        ...
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            if tag == "delete":
                ...
                changes.append(
                    f"Changed: '{' '.join(old_words[i1:i2])}' → '{' '.join(new_words[j1:j2])}'"
                )
