...
IMPERSONATION_TTL_SECONDS = int(
    os.getenv("IMPERSONATION_TTL_SECONDS", "1800")
)  # 30 minutes
IMPERSONATION_RATE_LIMIT_ADMIN_PER_HOUR = int(
    os.getenv("IMPERSONATION_RATE_LIMIT_ADMIN_PER_HOUR", "10")
)
IMPERSONATION_RATE_LIMIT_TARGET_PER_DAY = int(
    os.getenv("IMPERSONATION_RATE_LIMIT_TARGET_PER_DAY", "5")
)
IMPERSONATION_STRICT_IP_BINDING = (
    os.getenv("IMPERSONATION_STRICT_IP_BINDING", "true").lower() == "true"
)
...
def _validate_admin_user(admin_user: User) -> None:
    ...
    if not is_admin_user(admin_user):
        raise ImpersonationNotAllowedError(
            "Only verified admins can start impersonation sessions"
        )
    ...
    target_recent_sessions = (
        ...
        .filter(
            and_(
                ...
                > ImpersonationSession.started_at + timedelta(minutes=1),
            )
        )
        .count()
    )
...
def start_impersonation_session(
    ...
    try:
        ...
        if existing_session:
            ...
            logger.info(
                f"Ended existing session {existing_session.id} for admin {admin_user.id}"
            )

        # Create new session
        expires_at = datetime.now(timezone.utc) + timedelta(
            seconds=IMPERSONATION_TTL_SECONDS
        )
        ...
        logger.info(
            f"Started impersonation session {session.id}: admin {admin_user.email} -> target {target_user.email}"
        )
        return session
...
def end_impersonation_session(
    db: Session, session_id: str, end_reason: str = "ended_by_admin"
) -> bool:
    """
    ...
    try:
        ...
        logger.info(
            f"Ended impersonation session {session_id}: reason={end_reason}, duration={duration_seconds}s"
        )
        return True
...
def get_active_session_for_admin(
    db: Session, admin_id: str
...
def validate_session(
    ...
    if IMPERSONATION_STRICT_IP_BINDING and admin_ip and session.admin_ip:
        if admin_ip != session.admin_ip:
            logger.warning(
                f"IP mismatch for session {session_id}: expected {session.admin_ip}, got {admin_ip}"
            )
            return None
...
def get_active_sessions(
    db: Session, admin_id: Optional[str] = None, limit: int = 100, offset: int = 0
    ...
    query = db.query(ImpersonationSession).filter(
        and_(
            ImpersonationSession.ended_at.is_(None), ImpersonationSession.revoked == False
        )
    )
...
def revoke_session(db: Session, session_id: str, revoking_admin: User) -> bool:
    ...
    try:
        ...
        log_admin_action(
            ...
            target_user_id=session.target_user_id,
            details={"session_id": session.id, "original_admin_id": session.admin_id},
        )

        logger.info(
            f"Revoked impersonation session {session_id} by admin {revoking_admin.email}"
        )
        return True
