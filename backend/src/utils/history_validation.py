...
def validate_cv_data(cv_data: Dict[str, Any]) -> List[str]:
    ...
    if data_size > max_size:
        errors.append(f"CV data too large ({data_size} bytes). Maximum allowed: {max_size} bytes")
    ...
    array_sections = [
        ...
        'certifications', 'awards', 'publications', 'volunteer_experience'
    ]
...
def validate_description_and_label(description: str = None, label: str = None) -> List[str]:
    """
...
class ValidatedCreateHistoryRequest(BaseModel):
    ...
    change_type: ChangeType = Field(..., description="Type of change that triggered this snapshot")
    description: Optional[str] = Field(None, description="Human-readable description", max_length=1000)
    label: Optional[str] = Field(None, description="Optional user-provided label", max_length=255)
