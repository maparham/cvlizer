...
class TestJobFitAnalysisWithCachedTokens:
    """Test suite for job fit analysis with cached tokens."""

    @pytest.mark.asyncio
    @patch("src.services.ai_service.job_fit.get_openai_client")
    async def test_job_fit_analysis_saves_cached_tokens(
        self,
        mock_get_client,
        client,
        test_user,
        ...
        """Test that job fit analysis saves cached tokens to database."""
        # Arrange
        mock_client = Mock()
        ...
        mock_response.output_parsed.model_dump.return_value = {
            ...
            "suggested_improvements": ["Add cloud experience", "Quantify achievements"],
        ...
        mock_response.usage.input_tokens_details.cached_tokens = 500

        mock_client.chat.completions.create.return_value = mock_response

        # Act: Generate job fit analysis
        response = client.post(
            f"/api/ai/job-fit/{test_cv.id}",
            headers=auth_headers,
            json={"job_description_id": str(test_job_description.id)},
        )
        ...
        # Check that usage log was created with cached tokens
        usage_log = (
            db_session.query(AIUsageLog)
            .filter_by(user_id=test_user.id, operation_type="job_fit_analysis")
            .first()
        )
        assert usage_log is not None
        assert usage_log.cached_tokens == 500
        assert usage_log.prompt_tokens == 800
        assert usage_log.completion_tokens == 400

    @pytest.mark.asyncio
    @patch("src.services.ai_service.job_fit.get_openai_client")
    async def test_repeated_job_fit_analysis_benefits_from_caching(
        self,
        mock_get_client,
        client,
        test_user,
        ...
        """Test that repeated job fit analyses benefit from prompt caching."""
        # Arrange
        mock_client = Mock()
        mock_get_client.return_value = mock_client
        ...
        mock_response_1.output_parsed = Mock()
        mock_response_1.output_parsed.model_dump.return_value = {
            "confidence_score": 80,
        ...
        mock_response_1.usage.input_tokens = 800
        mock_response_1.usage.output_tokens = 400
        mock_response_1.usage.input_tokens_details = Mock()
        mock_response_1.usage.input_tokens_details.cached_tokens = 0

        # Second call: With cache (70% hit rate)
        mock_response_2 = Mock()
        mock_response_2.output_parsed = Mock()
        mock_response_2.output_parsed.model_dump.return_value = {
            "confidence_score": 85,
            "fit_analysis": "Great fit",
            "key_matches": ["Python", "FastAPI"],
            "missing_skills": ["Docker"],
            "suggested_improvements": ["Add projects"],
            "strengths": ["API design"],
            "weaknesses": ["DevOps"],
        }
        mock_response_2.usage = Mock()
        mock_response_2.usage.input_tokens = 800
        mock_response_2.usage.output_tokens = 400
        mock_response_2.usage.input_tokens_details = Mock()
        mock_response_2.usage.input_tokens_details.cached_tokens = 560  # 70% cache
        ...
        # First call: No cache
        assert usage_logs[0].cached_tokens == 0
        assert usage_logs[0].estimated_cost > usage_logs[1].estimated_cost

        # Second call: With cache (lower cost)
        assert usage_logs[1].cached_tokens == 560
        assert usage_logs[1].estimated_cost < usage_logs[0].estimated_cost
