...
def test_cv(db_session, test_user):
    ...
    cv = CV(
        ...
        filename="test.pdf",
        parsed_data={
            "personal_info": {"full_name": "Test User", "email": "test@example.com", "location": "US"},
...
class TestJobFitApprovalIntegration:
    ...
    def test_approve_draft_with_valid_data(self, db_session, test_cv, test_job_description):
        ...
        assert test_cv.parsed_data["why_good_fit"]["content"] == "This is a great fit because..."

        # Verify draft was deleted
        deleted_draft = db_session.query(AIDraft).filter(AIDraft.id == draft.id).first()
        assert deleted_draft is None

    def test_approve_draft_missing_confidence_score(self, db_session, test_cv, test_job_description):
    ...
    def test_approve_draft_with_content_mapping(self, db_session, test_cv, test_job_description):
        ...
        db_session.refresh(test_cv)
        assert test_cv.parsed_data["why_good_fit"]["content"] == "I'm a good fit because of my Python expertise"
        assert test_cv.parsed_data["why_good_fit"]["fit_analysis"] == "I'm a good fit because of my Python expertise"

    def test_approve_draft_updates_section_order(self, db_session, test_cv, test_job_description):
        ...
        why_good_fit_section = next((s for s in sections if s["type"] == "why_good_fit"), None)
    ...
    def test_approve_draft_with_generation_error(self, db_session, test_cv, test_job_description):
