...
class TestCVDiffService:
    ...
    def test_work_experience_field_modification(self):
        ...
        new_cv = self.sample_old_cv.copy()
        new_cv["work_experience"] = [
            item.copy() for item in self.sample_old_cv["work_experience"]
        ]
        ...
        assert change["section"] == "work_experience"
        assert (
            'Position changed from "Software Developer" to "Senior Software Developer"'
    ...
    def test_text_field_diff(self):
        ...
        new_cv["work_experience"][0][
            "description"
        ] = "Developed advanced web applications using React, Node.js, and modern frameworks. Led a team of 3 developers."
    ...
    def test_professional_summary_text_diff(self):
        ...
        new_cv["professional_summary"][
            "content"
        ] = "Highly experienced software developer with 7 years of experience in full-stack development."
    ...
    def test_section_display_names(self):
        ...
        assert (
            self.diff_service._get_section_display_name("work_experience")
            == "Work Experience"
        )
        assert (
            self.diff_service._get_section_display_name("personal_info")
            == "Personal Information"
        )
        assert (
            self.diff_service._get_section_display_name("professional_summary")
    ...
    def test_field_display_names(self):
        ...
        assert (
            self.diff_service._get_field_display_name("field_of_study")
            == "Field of Study"
        )
    ...
    def test_item_display_names(self):
        ...
        assert (
            self.diff_service._get_item_display_name(work_item, "work_experience")
            == "Developer at TechCorp"
        )

        edu_item = {"institution": "MIT", "degree": "PhD"}
        assert (
            self.diff_service._get_item_display_name(edu_item, "education")
            == "PhD from MIT"
        )

        project_item = {"name": "My Project"}
        assert (
            self.diff_service._get_item_display_name(project_item, "projects")
            == "My Project"
        )
    ...
    def test_text_field_detection(self):
        ...
        assert self.diff_service._is_text_field(
            "description",
            "Short desc",
            "This is a much longer description with substantial content.",
        )
        assert self.diff_service._is_text_field(
            "content",
            "Brief summary",
            "This is a comprehensive professional summary with detailed information.",
        )

        # Should not detect simple fields as text fields
        assert not self.diff_service._is_text_field(
    ...
    def test_word_diff_generation(self):
        ...
        assert any(
            "Changed:" in change and "developer" in change and "engineer" in change
    ...
    def test_value_formatting(self):
        ...
        assert (
            self.diff_service._format_value_for_display(["item1", "item2"]) == "2 items"
        )
    ...
    def test_complex_nested_changes(self):
        ...
        new_cv["work_experience"][0][
            "description"
        ] = "Led development of enterprise applications using modern technologies and agile methodologies."
    ...
    def test_summary_generation(self):
        ...
        assert (
            self.diff_service._generate_summary([{"type": "test1"}, {"type": "test2"}])
            == "2 Changes"
        )
        assert (
            self.diff_service._generate_summary([{"type": f"test{i}"} for i in range(5)])
            == "5 Changes"
        )
...
class TestCVDiffServiceIntegration:
    ...
    def test_move_and_edit_detection(self):
        ...
        assert (
            "Engineer" in change["description"]
            and "Senior Engineer" in change["description"]
        )
