...
class TestDateOrderValidation:
    ...
    def test_valid_date_order_work_experience(self):
        ...
        date_errors = [
            e for e in errors if "start date must be before end date" in e.lower()
        ]
    ...
    def test_invalid_date_format_skips_validation(self):
        ...
        assert (
            len(date_errors) == 0
        ), "Should skip date order validation for invalid date formats"
    ...
    def test_missing_end_date_skips_validation(self):
        ...
        assert (
            len(date_errors) == 0
        ), "Should skip date order validation when end_date is missing"
