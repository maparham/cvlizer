...
class TestPhantomDiffAPIIntegration:
    ...
    def test_diff_api_no_phantom_deletions(self, mock_get_db, mock_get_current_user):
        ...
        mock_db.query.side_effect = [
            ...
            MagicMock(
                filter=lambda *args: MagicMock(
                    order_by=lambda *args: MagicMock(
                        all=lambda: [mock_compare_entry, mock_target_entry]
                    )
                )
            ),
        ]
        ...
        try:
            ...
            assert not any(
                "education" in change.section.lower() for change in result.changes
            )
    ...
    def test_business_validation_consistency(self):
        ...
        ), "Education without start_date should be removed"
        ...
        assert (
            len(education_errors) == 0
        ), f"Should not have education validation errors: {education_errors}"
...
class TestDiffServiceRobustness:
    ...
    def test_diff_with_missing_sections(self):
        ...
        new_data = {
            "work_experience": [
                {"id": "work_1", "position": "Senior Dev", "company": "Co"}
            ]
    ...
    def test_diff_with_reordered_items(self):
        ...
        content_changes = [
            c for c in result["changes"] if c["type"] in ["item_added", "item_removed"]
        ]
        assert (
            len(content_changes) == 0
        ), "Reordering should not show as additions/removals"
