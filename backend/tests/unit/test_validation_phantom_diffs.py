...
class TestPhantomDiffPrevention:
    ...
    def test_education_entries_strict_validation(self):
        ...
        # Act
        cleaned_data = CVDataValidator.clean_empty_entries(self.cv_with_incomplete_education)
        
        # Assert - only complete entries should be preserved
        assert len(cleaned_data['education']) == 1, "Only complete education entries should be preserved"
    ...
    def test_work_experience_edit_no_phantom_education_deletions(self):
        ...
        old_cleaned = CVDataValidator.clean_empty_entries(self.cv_with_incomplete_education)
        ...
        assert diff_result['total_changes'] == 1, f"Expected 1 change, got {diff_result['total_changes']}"
        
        change = diff_result['changes'][0]
        assert change['section'] == 'work_experience', "Change should be in work experience"
        assert 'position' in change['description'].lower(), "Change should be about position"
        assert 'backend developer' in change['description'].lower(), "Should show position change"
        
        # Verify no education changes
        education_changes = [c for c in diff_result['changes'] if c['section'] == 'education']
        assert len(education_changes) == 0, "Should not have any phantom education deletions"
    ...
    def test_business_validation_requires_start_date(self):
        ...
        assert len(cleaned['education']) == 0, "Education without start_date should be removed during cleaning"
    ...
    def test_empty_education_entries_are_removed(self):
        ...
        # Assert
        assert len(cleaned_data['education']) == 1, "Only complete education entry should be kept"
    ...
    def test_work_experience_still_requires_all_fields(self):
        ...
        # Assert
        assert len(cleaned_data['work_experience']) == 1, "Incomplete work experience should be removed"
    ...
    def test_diff_consistency_after_multiple_saves(self):
        ...
        assert diff_1_to_2['total_changes'] == 0, "No changes should occur between save cycles"
        assert diff_2_to_3['total_changes'] == 0, "No changes should occur between save cycles"
        
        # Verify education count remains stable
        assert len(data_after_save1['education']) == len(data_after_save2['education']) == len(data_after_save3['education'])
    ...
    def test_regression_specific_case(self):
        ...
        after_edit['work_experience'][0]['position'] = 'Backend Developeree'  # Only change
        ...
        first_save_cleaned = CVDataValidator.clean_empty_entries(before_edit)  # Removes incomplete education
        ...
        diff_result = self.diff_service.compute_diff(first_save_cleaned, second_save_cleaned)
        
        # Assert - should only show the work experience change, no phantom education deletions
        assert diff_result['total_changes'] == 1, f"Expected exactly 1 change, got {diff_result['total_changes']}"
        ...
        education_changes = [c for c in diff_result['changes'] if c['section'] == 'education']
        assert len(education_changes) == 0, "Should have no education changes between properly cleaned versions"
