...
const mockGetDiff = backendHistoryService.getDiff as jest.MockedFunction<
  typeof backendHistoryService.getDiff
>;
...
describe("SimpleCVDiffViewer", () => {
      ...
      changes: [
        {
          ...
          description:
            'Work Experience: Position changed from "Developer" to "Senior Developer"',
          details: [],
          icon: "edit",
          color: "warning",
        },
  ...
  it("should display text diff with inline highlighting", async () => {
      ...
      changes: [
        {
          ...
          text_diff: {
            inline_diff:
              'Original text with <span style="background-color: #c8e6c9; font-weight: bold;">added content</span>',
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Professional Summary: Content text updated"),
      ).toBeInTheDocument();
      expect(screen.getByText("18 characters added")).toBeInTheDocument();
      expect(
        screen.getByText("Text with changes highlighted:"),
      ).toBeInTheDocument();
    });

    // Check that inline diff HTML is rendered
    const diffContainer = screen.getByText(
  ...
  it("should display multiple changes", async () => {
      ...
      changes: [
        {
          ...
          description:
            'Personal Information: Location changed from "New York, NY" to "Los Angeles, CA"',
          details: [],
          icon: "edit",
          color: "warning",
        },
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Added Tech Lead at StartupCorp/),
      ).toBeInTheDocument();
    });
  });
  ...
  it("should display no changes message", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("No changes detected between these versions."),
      ).toBeInTheDocument();
    });
  });
