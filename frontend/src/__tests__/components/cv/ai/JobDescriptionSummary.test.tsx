...
const mockUseVisibleJobDescriptions = useVisibleJobDescriptions as jest.MockedFunction<typeof useVisibleJobDescriptions>;
...
describe('JobDescriptionSummary', () => {
  ...
  describe('Rendering States', () => {
    it('renders "No Job Description Yet" state when no job descriptions exist', () => {
      ...
      expect(screen.getByText('Add a job description to get personalized AI suggestions and enhance your CV')).toBeInTheDocument();
  ...
  describe('User Interactions', () => {
    ...
    it('shows success notification when hiding job description', () => {
      ...
      expect(mockShowSuccess).toHaveBeenCalledWith('Job description removed from sidebar');
    });
    ...
    it('calls handleGenerateJobFit when Generate Job Fit Section button is clicked', async () => {
      ...
      await waitFor(() => {
        ...
        expect(mockShowSuccess).toHaveBeenCalledWith('Job fit analysis completed successfully');
      });
    });

    it('shows loading state during job fit generation', async () => {
      const mockCreateJobFitDraft = jest.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
      mockUseAIStore.mockReturnValue({
        ...defaultMockStore,
        createJobFitDraft: mockCreateJobFitDraft,
        jobDescriptions: [mockJobDescription],
  ...
  describe('Edit Form Functionality', () => {
    ...
    it('shows error when edit form submission fails', async () => {
      const mockCreateJobDescription = jest.fn().mockRejectedValue(new Error('Update failed'));
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('handles job fit generation error', async () => {
      const mockCreateJobFitDraft = jest.fn().mockRejectedValue(new Error('Generation failed'));
      const mockShowError = jest.fn();
      mockUseAIStore.mockReturnValue({
        ...defaultMockStore,
        createJobFitDraft: mockCreateJobFitDraft,
      ...
      await waitFor(() => {
        expect(mockShowError).toHaveBeenCalledWith('Error', 'Generation failed');
      });
    });
  });
  ...
  describe('URL Handling', () => {
    it('opens URL in new tab when URL chip is clicked', () => {
      ...
      expect(mockOpen).toHaveBeenCalledWith('https://example.com/job', '_blank', 'noopener,noreferrer');
    });
  });
