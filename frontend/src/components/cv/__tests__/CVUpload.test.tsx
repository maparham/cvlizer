...
const mockValidateCVFile = require("../../../utils/fileValidation")
  .validateCVFile as jest.Mock;
...
describe("CVUpload", () => {
  ...
  it("should render upload dialog", () => {
    render(
      <CVUpload open={true} onClose={mockOnClose} onSuccess={mockOnSuccess} />,
    );

    expect(screen.getByText("Upload CV")).toBeInTheDocument();
    expect(screen.getByText("Drag & drop your CV here")).toBeInTheDocument();
    expect(
      screen.getByText("Supported formats: PDF, DOC, DOCX (max 10MB)"),
    ).toBeInTheDocument();
  });

  it("should not render when closed", () => {
    render(
      <CVUpload open={false} onClose={mockOnClose} onSuccess={mockOnSuccess} />,
    );
  ...
  it("should handle drag and drop", async () => {
    ...
    const dropZone = screen
      .getByText("Drag & drop your CV here")
      .closest("div");
  ...
  it("should show error for invalid file type", async () => {
    ...
    await waitFor(
      () => {
        ...
        expect(
          screen.getByText("Please upload a PDF, DOC, or DOCX file"),
        ).toBeInTheDocument();
      },
      { timeout: 3000 },
    );
