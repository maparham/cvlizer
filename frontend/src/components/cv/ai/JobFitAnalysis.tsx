...
const JobFitAnalysis: React.FC<JobFitAnalysisProps> = ({
  ...
  const [pendingAnalysisResult, setPendingAnalysisResult] = useState<unknown>(null);
  ...
  const handleConfirmOverwrite = useCallback(() => {
    if (onAddToCV && pendingAnalysisResult && typeof pendingAnalysisResult === 'object' && pendingAnalysisResult !== null && 'fit_analysis' in pendingAnalysisResult) {
      const analysis = pendingAnalysisResult as { fit_analysis: string; confidence_score: number; key_matches: string[] };
      const whyGoodFitData = {
            ...
            title={
              <Box>
                ...
                <ul style={{ margin: 0, paddingLeft: '16px' }}>
                  ...
                  <li>Provide honest confidence score (1-100%) for role match</li>
                  <li>Identify key matches between your CV and job requirements</li>
                  <li>Highlight missing skills and suggested improvements</li>
                  <li>Always create positive content that defends your candidacy</li>
                </ul>
                <Typography variant="caption" sx={{ display: 'block', mt: 1, fontStyle: 'italic' }}>
                  Note: Creates a new CV section while providing honest assessment
                </Typography>
              </Box>
            }
            arrow
          ...
          <CardContent>
            <Box textAlign="center" py={2}>
              ...
              <Typography color="text.secondary" paragraph>
                Create an AI-powered "Why I'm a Good Fit" section that highlights your strengths and matches for this role.
              </Typography>
              <Button
                variant="contained"
                onClick={handleAnalyze}
                disabled={isGenerating || jobFitAnalysis.isAnalyzing}
                startIcon={isGenerating || jobFitAnalysis.isAnalyzing ? <CircularProgress size={20} /> : <AutoAwesomeIcon />}
                size="large"
              >
                {isGenerating || jobFitAnalysis.isAnalyzing ? 'Generating...' : 'Generate Job Fit Section'}
              </Button>
            </Box>
          </CardContent>
        </Card>
            ...
            <CardContent>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                ...
                <Chip
                  ...
                  color={getConfidenceColor(jobFitAnalysis.lastAnalysis.confidence_score)}
                  variant="filled"
                />
              </Box>
              <Typography variant="body2" color="text.secondary" gutterBottom>
                {getConfidenceLabel(jobFitAnalysis.lastAnalysis.confidence_score)}
              </Typography>
              <LinearProgress
                variant="determinate"
            ...
            <CardContent>
              ...
              <Box display="flex" gap={1} flexWrap="wrap">
                <Button
                  ...
                  variant="outlined"
                  startIcon={<CopyIcon />}
                  onClick={() => copyToClipboard(jobFitAnalysis.lastAnalysis!.fit_analysis)}
                >
                  Copy Content
                </Button>
              </Box>
          ...
          {jobFitAnalysis.lastAnalysis.key_matches.length > 0 && (
            <Card>
              <CardContent>
                ...
                <List dense>
                  {jobFitAnalysis.lastAnalysis.key_matches.map((match, index) => (
                    <ListItem key={index}>
                      <ListItemIcon>
              ...
              <Stack spacing={2}>
                {jobFitAnalysis.lastAnalysis.strengths.length > 0 && (
                  <Box>
                    ...
                    <List dense>
                      {jobFitAnalysis.lastAnalysis.strengths.map((strength, index) => (
                        <ListItem key={index}>
                ...
                {jobFitAnalysis.lastAnalysis.weaknesses.length > 0 && (
                  <Box>
                    ...
                    <List dense>
                      {jobFitAnalysis.lastAnalysis.weaknesses.map((weakness, index) => (
                        <ListItem key={index}>
          ...
          {jobFitAnalysis.lastAnalysis.missing_skills.length > 0 && (
            <Card>
              <CardContent>
                ...
                <Typography variant="body2" color="text.secondary" paragraph>
                  These skills are mentioned in the job description but not found in your CV:
                </Typography>
                <Box display="flex" gap={1} flexWrap="wrap">
                  {jobFitAnalysis.lastAnalysis.missing_skills.map((skill, index) => (
                    <Chip
                      key={index}
                      label={skill}
                      color="warning"
          ...
          {jobFitAnalysis.lastAnalysis.suggested_improvements.length > 0 && (
            <Card>
              <CardContent>
                ...
                <List dense>
                  {jobFitAnalysis.lastAnalysis.suggested_improvements.map((improvement, index) => (
                    <ListItem key={index}>
                      <ListItemIcon>
        ...
        <DialogContent>
          ...
          {existingWhyGoodFit && (existingWhyGoodFit as { content?: string }).content ? (
            <Box mb={3}>
              ...
              >
                <ReactMarkdown>{(existingWhyGoodFit as { content?: string }).content || ''}</ReactMarkdown>
              </Box>
            </Box>
          ) : null}

          {pendingAnalysisResult && (pendingAnalysisResult as { content?: string }).content ? (
            <Box>
              <Typography variant="subtitle2" gutterBottom>
                New Content:
              </Typography>
              ...
              >
                <ReactMarkdown>{(pendingAnalysisResult as { fit_analysis?: string }).fit_analysis || ''}</ReactMarkdown>
              </Box>
            </Box>
          ) : null}
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCancelOverwrite}>
            Cancel
          </Button>
          <Button
