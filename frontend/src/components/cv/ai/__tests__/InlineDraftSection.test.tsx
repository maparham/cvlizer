...
const mockDraft: DraftResponse = {
  ...
  draft_data: {
    fit_analysis: 'This is a test draft content that explains why the candidate is a good fit.',
    confidence_score: 85,
...
const mockApprovalResult = {
  ...
  cv: {
    ...
    parsed_data: {
      ...
      why_good_fit: {
        content: 'This is a test draft content that explains why the candidate is a good fit.',
        confidence_score: 85,
      },
    },
  },
...
describe('InlineDraftSection', () => {
  ...
  it('renders draft content correctly', () => {
    ...
    expect(screen.getByText(mockDraft.draft_data!.fit_analysis!)).toBeInTheDocument();
  ...
  it('handles draft rejection successfully', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockShowSuccess).toHaveBeenCalledWith('Draft rejected successfully');
  ...
  it('disables buttons during approval process', async () => {
    ...
    mockApproveDraft.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
