...
function IndividualItemSection<T>({
  ...
  const {
    ...
    handleCancelEdit,
    handleUpdateItem
  } = useItemEditing<T>(title, onUnsavedChanges, unregisterIndividualItemEditing, onClose)
        ...
        <Box>
          ...
          {editingItemIndex !== null && editingItemIndex >= itemsData.length && editData && (
            <EditForm
              ...
              updateItem={(field, value) => handleUpdateItem(field as keyof T, value, itemsData)}
            />
          )}

          {/* Drag and Drop Context for reordering existing items */}
          <DragDropContext onDragStart={handleDragStart} onDragEnd={handleDragEndComplete}>
