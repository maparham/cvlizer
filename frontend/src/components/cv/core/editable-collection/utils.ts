 ...
 */
export const validateItem = <T>(item: T | null, requiredFields: (keyof T)[], sectionTitle?: string): boolean => {
  return CVValidationService.validateItem(item, requiredFields, sectionTitle || '')
}
 ...
 */
export const reorderItems = <T>(items: T[], sourceIndex: number, destinationIndex: number): T[] => {
