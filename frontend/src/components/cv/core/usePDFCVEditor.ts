...
export const usePDFCVEditor = ({
  ...
  const [pendingNavigation, setPendingNavigation] = useState<
    (() => void) | null
  >(null);
  const [editingIndividualItem, setEditingIndividualItem] =
    useState<EditingIndividualItem | null>(null);
  const [
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape" && (editingSection || editingIndividualItem)) {
        ...
        if (editingIndividualItem) {
          const hasCurrentSectionChanges = pendingChanges.has(
            editingIndividualItem.sectionId,
          );
  ...
  const registerIndividualItemEditing = useCallback(
    (
      ...
      if (currentEditingItem) {
        ...
        const hasRealDataChanges = pendingChanges.has(
          currentEditingItem.sectionId,
        );
  ...
  const getAvailableSectionsToAdd = () => {
    ...
    return AVAILABLE_SECTIONS.filter(
