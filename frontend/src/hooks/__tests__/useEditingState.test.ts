...
const mockUpdatePendingChanges = jest.fn(
  (sectionId: string, changes: unknown) => {
    if (
      changes &&
...
jest.mock("../../components/cv/core/useUnsavedChanges", () => ({
  useUnsavedChanges: () => ({
    get hasUnsavedChanges() {
      return mockHasUnsavedChanges;
    },
    get editingSections() {
      return mockEditingSections;
    },
    get pendingChanges() {
...
describe("useEditingState", () => {
  ...
  it("should handle requestIndividualItemCancel", () => {
    ...
    // Request cancel
    act(() => {
      result.current.requestIndividualItemCancel(
        "work_experience",
        mockOnCancel,
      );
    });
  ...
  describe("Discard Changes Flow", () => {
    ...
    it("should show dialog when editing another item with pending changes", () => {
      ...
      act(() => {
        registrationResult = result.current.registerIndividualItemEditing(
          "education",
          1,
          mockOnCancel2,
        );
  ...
  describe("Non-Array Section Editing", () => {
    ...
    it("should close professional_summary edit mode after discarding changes when editing elsewhere", () => {
      ...
      expect(result.current.editingSections.has("professional_summary")).toBe(
        false,
      );
