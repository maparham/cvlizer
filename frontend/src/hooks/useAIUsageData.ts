...
interface UseAIUsageDataReturn {
  ...
  handleGranularityChange: (granularity: 'day' | 'week' | 'month' | 'hour') => void
}
...
export const useAIUsageData = (): UseAIUsageDataReturn => {
  ...
  const [granularity, setGranularityState] = useState<'day' | 'week' | 'month' | 'hour'>('day')
  ...
  const loadAIUserUsage = async () => {
    try {
      const users = await getAIUsageByUser(dateRange.start, dateRange.end, 20, 0)
  ...
  const loadAIOperationUsage = async () => {
    try {
      const operations = await getAIUsageByOperation(dateRange.start, dateRange.end)
  ...
  const loadAITimeline = async () => {
    try {
      const timeline = await getAIUsageTimeline(dateRange.start, dateRange.end, granularity)
  ...
  const loadAILogs = async (filtersToUse = filters) => {
    try {
      ...
      const logs = await getAIUsageLogs(filtersWithDateRange, logsLimit, logsPage * logsLimit)
      setAiLogs(logs)
    } catch (err) {
  ...
  }, [dateRange.start, dateRange.end, granularity, filters.user_id, filters.operation_type, filters.success, logsPage, logsLimit])
  ...
  const handleDeleteAllLogs = async () => {
    try {
      ...
      setError(err.response?.data?.detail || 'Failed to delete all AI usage logs')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePaginationChange = async (page: number, rowsPerPage: number) => {
    ...
    try {
      ...
      const logs = await getAIUsageLogs(filtersWithDateRange, rowsPerPage, page * rowsPerPage)
      setAiLogs(logs)
    } catch (err) {
      console.error('Failed to load AI logs:', err)
    }
   ...
   */
  const handleGranularityChange = async (newGranularity: 'day' | 'week' | 'month' | 'hour') => {
    setGranularityState(newGranularity)
    // Load with new granularity directly to avoid stale state
    try {
      const timeline = await getAIUsageTimeline(dateRange.start, dateRange.end, newGranularity)
      setAiTimeline(timeline)
    } catch (err) {
      console.error('Failed to load AI timeline:', err)
    }
   ...
   * @param newGranularity - New granularity level
   */
  const setGranularity = (newGranularity: 'day' | 'week' | 'month' | 'hour') => {
    setGranularityState(newGranularity)
  ...
  useEffect(() => {
    ...
    loadAIUsageData()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isAuthenticated, navigate, dateRange.start, dateRange.end, granularity, logsPage, logsLimit])
