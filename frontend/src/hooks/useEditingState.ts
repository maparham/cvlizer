...
export const useEditingState = (
  ...
  const [editingState, dispatch] = useReducer(editingStateReducer, {
    mode: "idle",
  });
  ...
  const [showUnsavedChangesDialog, setShowUnsavedChangesDialog] =
    useState(false);
  const [
    pendingIndividualItemRegistration,
  ...
  const onUnsavedChanges = useCallback(
    (sectionId: string, hasChanges: boolean) => {
  ...
  const registerIndividualItemEditing = useCallback(
    (
      ...
      if (
        !isEditingItem(currentState) &&
        currentPendingChanges.size > 0 &&
        !skipDialog
      ) {
      ...
      if (isEditingItem(currentState)) {
        const hasRealDataChanges = currentPendingChanges.has(
  ...
  const unregisterIndividualItemEditing = useCallback(
    (_sectionId: string, _itemIndex: number) => {
      dispatch({ type: "CLOSE" });
    },
    [],
  );
  ...
  const requestIndividualItemCancel = useCallback(
    (sectionId: string, onCancel: () => void) => {
      // Use ref to get the LATEST state (avoid stale closure)
      const currentState = editingStateRef.current;
  ...
  const handleUnsavedChangesDialogConfirm = useCallback(() => {
    ...
    window.dispatchEvent(
      new CustomEvent("is-discarding-changes", {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener(
        "cv-editing-state-clear",
        handleEditingStateClear,
      );
    };
  }, [clearEditingState]);

  // Extract values from state machine for compatibility with existing components
  const editingSection = isEditingSection(editingState)
