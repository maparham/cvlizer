...
export const useKeyboardShortcuts = ({
  ...
  const handleEscapeKey = useCallback(() => {
    ...
    // Fallback: Look for cancel buttons in forms that are NOT managed by global state
      ...
      const simpleFormCancelButtons = Array.from(cancelButtons).filter(
        (button) => {
          ...
          const isInSimpleForm = htmlButton.closest(
            '.simple-form-section, [data-section-type="simple"], .form-section:not(.individual-item)',
          );
      ...
      const simpleFormCloseButtons = Array.from(closeButtons).filter(
        (button) => {
          const htmlButton = button as HTMLButtonElement;
          const isInSimpleForm = htmlButton.closest(
            '.simple-form-section, [data-section-type="simple"], .form-section:not(.individual-item)',
