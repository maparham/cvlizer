...
interface UseUserActionsReturn {
  ...
  loadUserActivities: (userId: string, page?: number, limit?: number, activityType?: string) => Promise<void>
...
export const useUserActions = (): UseUserActionsReturn => {
  ...
  const loadUserDetail = async (userId: string) => {
    try {
      ...
      throw new Error(err.response?.data?.detail || 'Failed to load user details')
    } finally {
  ...
  const loadUserActivities = async (userId: string, page: number = 0, limit: number = 50, activityType?: string) => {
    try {
      ...
      throw new Error(err.response?.data?.detail || 'Failed to load user activities')
    } finally {
      setActionLoading(null)
  ...
  const clearUserActivities = async (userId: string) => {
    try {
      ...
      throw new Error(err.response?.data?.detail || 'Failed to clear user activities')
    }
  }
  ...
  const loadUserErrors = async (userId: string) => {
    try {
      ...
      throw new Error(err.response?.data?.detail || 'Failed to load user errors')
    } finally {
      setActionLoading(null)
    }
  }
