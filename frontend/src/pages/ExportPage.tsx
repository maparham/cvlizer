...
export const ExportPage: React.FC = () => {
  ...
  >(undefined);
  /** Server value of the AI credit line setting; `undefined` until the CV loads */
  const [showAIAttribution, setShowAIAttribution] = useState<boolean | undefined>(
    undefined,
  );
  /** The credit line only renders when the AI-generated section is in the CV. */
  ...
  const handleToggleAIAttribution = useCallback(
    async (nextValue: boolean) => {
      if (!cvId) {
        return;
      }
      const previous = showAIAttribution;
      setShowAIAttribution(nextValue);
      try {
        await cvApi.patchAIAttribution(cvId, nextValue);
      } catch {
        setShowAIAttribution(previous);
      }
    },
    [cvId, showAIAttribution],
  );
  ...
  return (
    <Box sx={{ minHeight: "100vh", backgroundColor: "background.default" }}>
      ...
      <Container maxWidth="xl" sx={{ py: 4 }}>
        ...
        {hasAISection && showAIAttribution !== undefined && (
          <Box sx={{ mb: 3 }}>
            <FormControlLabel
              control={
                <Switch
                  checked={showAIAttribution}
                  onChange={(event) => handleToggleAIAttribution(event.target.checked)}
                />
              }
              label="Credit rahkar.pro for the AI-generated section"
            />
            <Typography variant="body2" color="text.secondary">
              Adds &ldquo;Section AI generated by rahkar.pro&rdquo; at the bottom of your
              exported CV. Turn it off to export without the note.
            </Typography>
          </Box>
        )}
