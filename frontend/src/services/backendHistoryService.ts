...
import api from './api'
import { applyRetentionPolicy, DEFAULT_RETENTION_POLICY, RetentionPolicy } from '../utils/historyRetention'
...
export class BackendCVHistoryService {
   ...
   */
  async getHistoryEntry(cvId: string, entryId: string): Promise<CVHistoryEntry | null> {
    try {
    ...
    if (fromStr !== toStr) {
      if (Array.isArray(fromSection) && Array.isArray(toSection)) {
        if (fromSection.length !== toSection.length) {
          changes.push(`Changed from ${fromSection.length} to ${toSection.length} items`)
        } else {
   ...
   */
  async getDiff(cvId: string, entryId: string, compareToId?: string, forcePrevious?: boolean): Promise<CVDiffResult> {
    try {
