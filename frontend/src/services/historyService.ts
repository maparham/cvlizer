...
export class CVHistoryService {
  ...
  createSnapshot(
    ...
    if (dataSize > this.config.maxSnapshotSize) {
      throw new Error(`Snapshot too large: ${dataSize} bytes exceeds limit of ${this.config.maxSnapshotSize} bytes`)
    }
   ...
   */
  generateDiff(fromEntry: CVHistoryEntry, toEntry: CVHistoryEntry): HistoryDiff {
    const changes = {
      added: [] as string[],
  ...
  private cleanupHistory(state: CVHistoryState): void {
    ...
    state.entries = [...keepAuto, ...keepManual]
