...
export function applyRetentionPolicy(
  ...
  const now = new Date()
  const cutoffDate = new Date(now.getTime() - (policy.maxAutoVersionAgeDays * 24 * 60 * 60 * 1000))
  ...
  const finalEntries = [...preserved, ...keptCandidates]
...
export function getRetentionStats(
  ...
  if (oldestAuto) {
    const cutoffDate = new Date().getTime() - (policy.maxAutoVersionAgeDays * 24 * 60 * 60 * 1000)
    const oldestTime = new Date(oldestAuto.timestamp).getTime()
    if (oldestTime > cutoffDate) {
      nextCleanupIn = Math.ceil((oldestTime - cutoffDate) / (24 * 60 * 60 * 1000))
    }
  }
