...
export const generateDeterministicId = (item: any, sectionType: string, index: number): string => {
  ...
  if (sectionType === 'work_experience' && item.company && item.start_date) {
    ...
    contentHash = `${item.organization}_${item.start_date}`.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase()
  } else if (sectionType === 'languages' && item.language && item.proficiency) {
    // Use language + proficiency (stable identifiers)
    contentHash = `${item.language}_${item.proficiency}`.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase()
  } else {
 ...
 */
export const ensureId = <T extends { id?: string }>(item: T, sectionType?: string): T & { id: string } => {
  if (item.id) {
