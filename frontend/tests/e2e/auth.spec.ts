...
test.describe("Authentication Flows", () => {
  test("1. Login with valid credentials and admin routing", async ({
    page,
  }) => {
  ...
  test("2. Login with invalid credentials shows error", async ({ page }) => {
    ...
    const errorVisible = await page
      .getByText(/incorrect|invalid|wrong|couldn't|failed/i)
    ...
    if (!errorVisible) {
      ...
      await expect(
        page.getByText(/incorrect|invalid|wrong|couldn't|failed/i),
      ).toBeVisible();
    }
  });
