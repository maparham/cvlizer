...
test.describe("Create and Delete CV Workflow", () => {
  test("should create a minimal CV and then delete it", async ({
    page,
    testUser,
  }) => {
    ...
    await page
      .getByRole("button", { name: /add this section to your cv/i })
      .first()
      .click();

    // Wait for section to be added
    await expect(
      page.getByText(/professional summary section added/i),
    ...
    const summaryText =
      "Experienced professional with a strong track record in delivering high-quality results. Passionate about continuous learning and applying innovative solutions to complex challenges.";
    await page
      .getByRole("textbox", { name: /your professional summary/i })
      .fill(summaryText);
    ...
    await expect(page.getByText(/professional summary saved/i)).toBeVisible({
      timeout: TEST_TIMEOUT,
    });

    // Step 9: Add Skills section
    const addButtons = page.getByRole("button", {
      name: /add this section to your cv/i,
    });
    await addButtons.nth(3).click(); // Skills section

    // Wait for section to be added
    await expect(page.getByText(/skills section added/i)).toBeVisible({
      timeout: TEST_TIMEOUT,
    });
    ...
    await expect(page.getByText(/technical skill added/i)).toBeVisible({
      timeout: TEST_TIMEOUT,
    });
    ...
    await page
      ...
      .click();
    await expect(page.getByText(/soft skill added/i)).toBeVisible({
      timeout: TEST_TIMEOUT,
    });
    ...
    await expect(page.getByTestId(`edit-cv-button-${cvId}`)).toBeVisible({
      timeout: DASHBOARD_REFRESH_TIMEOUT,
    });

    // Step 11: Find and delete the newly created CV using real CV ID
    await expect(page.getByTestId(`delete-cv-button-${cvId}`)).toBeVisible({
      timeout: TEST_TIMEOUT,
    });
    ...
    await expect(
      page.getByRole("dialog", { name: /delete cv/i }),
    ).toBeVisible();
    await expect(
    ...
    await expect(page.getByTestId(`delete-cv-button-${cvId}`)).not.toBeVisible({
      timeout: TEST_TIMEOUT,
    });
  });
});
