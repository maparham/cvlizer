...
test.describe("CV Editor - Export and Delete", () => {
  test("1. Export CV initiates download", async ({ page, testUser }) => {
    ...
    if (await exportButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      if (download) {
        console.log(
          `✓ Export initiated: ${await download.suggestedFilename()}`,
        );
      } else {
        // Export might open a dialog instead
        const exportDialog = page.getByRole("dialog");
        if (
          await exportDialog.isVisible({ timeout: 2000 }).catch(() => false)
        ) {
  ...
  test("2. Delete CV from editor with confirmation", async ({
    page,
    testUser,
  }) => {
    // Authentication handled by global-setup.ts
    ...
    if (await deleteMenuItem.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      await expect(page.getByTestId(`edit-cv-button-${cvId}`)).not.toBeVisible({
        timeout: TEST_TIMEOUT,
      });
      ...
      console.log(
        "⚠ Delete menu item in editor may require different selector",
      );
  ...
  test("3. Export shows appropriate message for new unsaved CV", async ({
    page,
    testUser,
  }) => {
    // Authentication handled by global-setup.ts
    ...
    if (await exportButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const errorMsg = await page
        .getByText(/save.*before.*export|add.*content/i)
