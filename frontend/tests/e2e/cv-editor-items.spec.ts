...
test.describe('CV Editor - Item CRUD Operations', () => {
  ...
  test('1. Add first Education item with all fields', async () => {
    ...
    await page.getByRole('textbox', { name: 'Description' }).fill('Research in Machine Learning');
  ...
  test('3. Edit existing Education item', async () => {
    ...
    await page.getByTestId('edit-education-item-0').click();
    await expect(page.getByRole('textbox', { name: 'Field of Study' })).toHaveValue('Artificial Intelligence');
  ...
  test('5. Add first Work Experience item', async () => {
    ...
    await page.getByRole('textbox', { name: 'Description' }).fill('Developed scalable backend systems');
  ...
  test('6. Add second Work Experience item', async () => {
    ...
    await page.getByRole('textbox', { name: 'Company *' }).fill('Meta Platforms');
  ...
  test('7. Edit existing Work Experience item', async () => {
    ...
    await descInput.fill('Led development of microservices architecture and mentored junior engineers');
  ...
  test('9. Cancel operation discards changes', async () => {
    ...
    await page.getByTestId('edit-education-item-0').click();
    await expect(page.getByRole('textbox', { name: 'Institution *' })).toHaveValue(originalValue);
  ...
  test('10. Required field validation prevents saving', async () => {
    ...
    if (!isDisabled) {
      ...
      await expect(page.getByText(/required|institution is required/i)).toBeVisible({ timeout: 2000 });
    } else {
  ...
  test('11. Item count updates correctly after all operations', async () => {
    ...
    await page.getByRole('heading', { name: 'Work Experience' }).scrollIntoViewIfNeeded();
    await expect(page.getByTestId('edit-work-experience-item-0')).toBeVisible();
    await expect(page.getByTestId('edit-work-experience-item-1')).not.toBeVisible();
  });
