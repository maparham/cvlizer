...
test.describe("CV Editor - Section Management", () => {
  ...
  test.beforeAll(async ({ browser }, testInfo) => {
    ...
    console.log(
      `✓ Created test CV: ${cvId} (will be reused for all section tests)`,
    );
  });
  ...
  test("1. Add Education section from sidebar", async () => {
    ...
    await expect(page.getByText(/education section added/i)).toBeVisible({
      timeout: 3000,
    });
  ...
  test("2. Add Work Experience section", async () => {
    ...
    await expect(page.getByText(/work experience section added/i)).toBeVisible({
      timeout: 3000,
    });
    await expect(
      page.getByTestId("add-new-work-experience-button"),
    ).toBeVisible();
  });
  ...
  test("8. All added sections are still visible (persistence check)", async () => {
    ...
    await expect(
      page.getByRole("heading", { name: "Work Experience", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Skills", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Professional Summary", exact: true }),
    ).toBeVisible();
    await expect(
