...
test.describe("Dashboard - CV Management", () => {
  ...
  test.beforeAll(async ({ browser }, testInfo) => {
    ...
    currentTestUser = {
      ...
      email: isUser2
        ? "mahmoud.shahrood+testuser2@gmail.com"
        : "mahmoud.shahrood+testuser1@gmail.com",
  ...
  test("1. Create CV from scratch", async () => {
    ...
    await page
      .getByTestId("personal-info-full-name-input")
      .locator("input")
      .fill(currentTestUser.displayName);
  ...
  test("3. Rename CV inline", async () => {
    ...
    if (
      await titleElement.isVisible({ timeout: TEST_TIMEOUT }).catch(() => false)
    ) {
  ...
  test("4. Duplicate existing CV", async () => {
    ...
    const cvCard = page
      .locator(`[data-testid="edit-cv-button-${cvId}"]`)
      .locator("..");
    ...
    const cvCards = await page
      .locator('[data-testid^="edit-cv-button-"]')
      .count();
  ...
  test("5. Delete CV with confirmation", async () => {
    ...
    await expect(
      deleteDialog.getByText(/are you sure you want to delete/i),
    ).toBeVisible();
    ...
    await expect(
      page.getByTestId(`edit-cv-button-${cvToDelete}`),
  ...
  test.afterAll(async () => {
    ...
    try {
      ...
      for (const cvId of createdCVIds) {
        ...
        if (
          await deleteButton.isVisible({ timeout: 2000 }).catch(() => false)
        ) {
