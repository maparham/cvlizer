...
async function setupDashboardWithMultipleCVs(
  page: Page,
...
test.describe("Dashboard - Search and Filter", () => {
  ...
  test.beforeAll(async ({ browser }, testInfo) => {
    ...
    cvIds = await setupDashboardWithMultipleCVs(
      testPage,
      currentTestUser.displayName,
  ...
  test("1. Search CVs by name finds matching results", async () => {
    ...
    if (await searchInput.isVisible({ timeout: 1000 }).catch(() => false)) {
      await searchInput
        .locator("input")
        .fill(`${currentTestUser.displayName} CV1`);
