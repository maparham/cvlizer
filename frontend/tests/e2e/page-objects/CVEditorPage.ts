...
export class CVEditorPage {
  ...
  constructor(page: Page) {
    ...
    this.personalInfoFullNameInput = page
      .getByTestId("personal-info-full-name-input")
      .locator("input");
    this.personalInfoEmailInput = page
      .getByTestId("personal-info-email-input")
      .locator("input");
    this.personalInfoPhoneInput = page
      .getByTestId("personal-info-phone-input")
      .locator("input");

    // Work Experience Section
    this.addNewWorkExperienceButton = page.getByTestId(
      "add-new-work-experience-button",
    );
    this.editWorkExperienceButton = (index: number) =>
      page.getByTestId(`edit-work-experience-item-${index}`);
    this.deleteWorkExperienceButton = (index: number) =>
      page.getByTestId(`delete-work-experience-item-${index}`);
    this.saveWorkExperienceButton = page.getByTestId(
      "save-work-experience-button",
    );
    this.cancelWorkExperienceButton = page.getByTestId(
      "cancel-work-experience-button",
    );

    // Education Section
    this.addNewEducationButton = page.getByTestId("add-new-education-button");
    this.editEducationButton = (index: number) =>
      page.getByTestId(`edit-education-item-${index}`);
    this.deleteEducationButton = (index: number) =>
      page.getByTestId(`delete-education-item-${index}`);
    ...
    this.editProjectsButton = (index: number) =>
      page.getByTestId(`edit-projects-item-${index}`);
    this.deleteProjectsButton = (index: number) =>
      page.getByTestId(`delete-projects-item-${index}`);
  ...
  async waitForLoad() {
    ...
    await expect(
      this.page.locator('[data-testid="cv-content-area"]'),
  ...
  async goBackWithUnsavedChanges(leaveAnyway: boolean = false) {
    ...
    const dialogVisible = await this.unsavedChangesDialog.isVisible({
      timeout: 1000,
    });
  ...
  async addWorkExperience(data: {
    ...
    if (data.startDate) {
      const startDateGroup = this.page.getByRole("group", {
        name: "Start Date *",
      });
  ...
  async editWorkExperience(
    ...
    if (data.position) {
      const positionInput = this.page.getByRole("combobox", {
        name: "Position",
      });
    ...
    if (data.company) {
      const companyInput = this.page.getByRole("textbox", {
        name: "Company *",
      });
