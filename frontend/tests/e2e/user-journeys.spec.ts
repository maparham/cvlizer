...
test.describe('Critical User Journeys', () => {
  ...
  test('1. Complete CV creation journey: Create → Add sections → Fill data', async ({ page, testUser }) => {
    ...
    await page.getByRole('combobox', { name: 'Position' }).fill('Product Manager');
    await page.keyboard.press('Tab');
    ...
    await page.getByRole('textbox', { name: 'Description' }).fill('Led product strategy for AWS services');

    await page.getByTestId('save-work-experience-button').click();
    await expect(page.getByTestId('edit-work-experience-item-0')).toBeVisible({ timeout: TEST_TIMEOUT });
    ...
    await expect(page.getByRole('heading', { name: 'Professional Summary' }).first()).toBeVisible();
  ...
  test('3. CV lifecycle journey: Create → Edit → Duplicate → Delete original', async ({ page, testUser }) => {
    ...
    const cvCard = page.locator(`[data-testid="edit-cv-button-${realCvId}"]`).locator('..');
    ...
    // Delete the original
    await expect(page.getByTestId(`delete-cv-button-${realCvId}`)).toBeVisible({ timeout: TEST_TIMEOUT });
    ...
    console.log(`✓ Completed CV lifecycle: create → duplicate → delete original`);
  });

  test('4. Multi-section workflow: Build complete CV incrementally', async ({ page }) => {
    // Authentication handled by global-setup.ts
    await page.goto('/');
    ...
    // Wait for form to be visible
    await expect(page.getByRole('combobox', { name: 'Degree' })).toBeVisible({ timeout: TEST_TIMEOUT });
    ...
    await page.getByRole('textbox', { name: 'Institution *' }).fill('UC Berkeley');
    ...
    // Wait for form to be visible
    await expect(page.getByRole('combobox', { name: 'Position' })).toBeVisible({ timeout: TEST_TIMEOUT });
    ...
    await expect(page.getByRole('heading', { name: 'Skills' })).toBeVisible({ timeout: TEST_TIMEOUT });
